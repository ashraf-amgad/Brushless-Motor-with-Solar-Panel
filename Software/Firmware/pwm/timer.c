...
void Timer_Disable(u8 timer)
{
	switch (timer)
	{
		case (TIMER_0): 
			#if (TIMER_0_INIT == TIMER_INIT) 
				/* stop timer */
				CLR_BITS(TIMER_TCCR0 , TIMER_TCCR0_CS00 | TIMER_TCCR0_CS01 | TIMER_TCCR0_CS02);
				/* disconnect pwm pin */
				CLR_BITS(TIMER_TCCR0 , TIMER_TCCR0_COM00 | TIMER_TCCR0_COM01);
			#endif			
		break;
		...
		case (TIMER_2):
			#if (TIMER_2_INIT == TIMER_INIT)
				/* stop timer */
				CLR_BITS(TIMER_TCCR2 , TIMER_TCCR2_CS20 | TIMER_TCCR2_CS21 | TIMER_TCCR2_CS22);
				/* disconnect pwm pin */
				CLR_BITS(TIMER_TCCR2 , TIMER_TCCR2_COM20 | TIMER_TCCR2_COM21);
			#endif
		break;
...
void PWM_OutputPinConnect(u8 timer)
{
	switch (timer)
	{
		case (TIMER_0):
			#if (TIMER_0_INIT == TIMER_INIT)
				#if (TIMER_0_MODE == TIMER_MODE_CTC)
					/*** SET OC0 MODE ***/
					#if   (TIMER_0_OC0_MODE == TIMER_OCn_NON_PWM_DISCONNECT) 
						CLR_BITS(TIMER_TCCR0 , TIMER_TCCR0_COM00 | TIMER_TCCR0_COM01);
					#elif (TIMER_0_OC0_MODE == TIMER_OCn_NON_PWM_TOGGLE)
		...
		case (TIMER_2):
			#if (TIMER_2_INIT == TIMER_INIT)
				#if (TIMER_2_MODE == TIMER_MODE_CTC)
			/*** SET OC0 MODE ***/
			#if   (TIMER_2_OC2_MODE == TIMER_OCn_NON_PWM_DISCONNECT) 
				CLR_BITS(TIMER_TCCR2 , TIMER_TCCR2_COM20 | TIMER_TCCR2_COM21);
			#elif (TIMER_2_OC2_MODE == TIMER_OCn_NON_PWM_TOGGLE)
